...
const Billing = () => {
  ...
  useEffect(() => {
    if (selectedCustomer) { setCustomerSuggestions([]); return; }
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      if (customerRef.current && !customerRef.current.contains(e.target)) setCustomerSuggestions([]);
    };
  ...
  const increaseQty = (id) => {
    setCart(cart.map(item => {
      if (item.id === id) {
        if (item.qty >= item.maxStock) { alert(`Only ${item.maxStock} units available!`); return item; }
  ...
  const handleCheckout = async () => {
    ...
    if (!finalAmount) { setError('Please enter final amount to charge.'); return; }
    setError('');
    if (paymentMethod === 'CREDIT' && !customerId.trim()) {
      setError('Customer ID is required for Store Credit (Udhaar) sales.'); return;
    }
    const parsedFinal = parseFloat(finalAmount);
    const saleRequest = {
      customerName, customerMobile, paymentMethod,
      saleItems: cart.map(item => ({ medicineId: item.id, batchNo: item.batchNo, quantity: item.qty })),
      emergencySale: isEmergency,
      finalAmount: parsedFinal,
      roundOff: parsedFinal - subtotal,
      ...(paymentMethod === 'CREDIT' && { customerId: customerId.trim() })
    };
    try {
      const response = await api.post('/sales', saleRequest);
      setLastSale({ ...response.data, finalAmount: parsedFinal, roundOff: parsedFinal - subtotal });
      setCart([]); setCustomerName(''); setCustomerMobile('');
      setCustomerId(''); setFinalAmount(''); setRoundOff(0); setSelectedCustomer(null);
    } catch (err) {
  ...
  return (
    <div style={{ padding: isMobile ? '16px' : '40px', backgroundColor: '#0f172a', minHeight: '100vh', boxSizing: 'border-box' }}>
      ...
      <header style={{
        ...
        <div
          ...
          style={{
            ...
            border: `2px solid ${isEmergency ? '#f59e0b' : '#334155'}`,
            color: isEmergency ? '#f59e0b' : '#94a3b8',
      ...
      {/* ── MAIN LAYOUT: left (cart) + right (summary) ── */}
      <div style={{
        display: 'flex',
        flexDirection: isMobile ? 'column' : 'row',
        gap: '20px',
        ...
        {/* ── LEFT PANEL ── */}
        <div style={{ flex: 2, width: '100%', minWidth: 0 }}>
          ...
          <div style={{
            ...
            {/* Customer Name */}
            <div style={{ flex: 1, position: 'relative' }} ref={customerRef}>
              <div style={inputGroup}>
                ...
                <input
                  ...
                  style={cleanInput}
                />
                {selectedCustomer && <span style={linkedBadgeStyle}>Linked ✔</span>}
              </div>
              {customerSuggestions.length > 0 && (
                <div style={resultsDropdown}>
                  {customerSuggestions.map(cust => (
                    <div key={cust.id} onMouseDown={() => handleSelectCustomer(cust)} style={customerSuggestItem}>
                      <div>
                        <div style={{ fontWeight: 'bold', fontSize: '13px' }}>{cust.name}</div>
                        <div style={{ fontSize: '11px', color: '#94a3b8' }}>{cust.mobile || 'No Mobile'}</div>
                      </div>
          ...
          {/* Udhaar Customer ID */}
          {isCreditSale && (
            <div style={{ ...inputGroup, marginBottom: '16px', border: '1px solid #f59e0b' }}>
              <Hash size={16} color="#f59e0b" style={{ flexShrink: 0 }} />
              <input
          ...
          {/* Medicine Search */}
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', backgroundColor: '#1e293b', padding: '14px', borderRadius: '12px', border: '1px solid #334155', marginBottom: '16px' }} ref={searchRef}>
            <Search color="#94a3b8" size={20} style={{ flexShrink: 0 }} />
            <input
              type="text"
              placeholder={isMobile ? 'Search medicine...' : 'Type medicine name (e.g. Azi, Dolo, Para)...'}
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              onFocus={() => searchResults.length > 0 && setShowDropdown(true)}
              style={{ background: 'none', border: 'none', color: '#f1f5f9', width: '100%', marginLeft: '10px', outline: 'none', fontSize: '14px' }}
            />
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: '11px', color: med.stockQuantity < 20 ? '#ef4444' : '#94a3b8' }}>
                        Batch: {med.batchNo} | Exp: {med.expiryDate ? new Date(med.expiryDate).toLocaleDateString('en-IN') : 'N/A'} | Stock: {med.stockQuantity}
                      </div>
                    </div>
          ...
          {/* ── CART TABLE ── */}
          <div style={{ backgroundColor: '#1e293b', borderRadius: '16px', padding: isMobile ? '12px' : '20px', border: '1px solid #334155' }}>
            {cart.length === 0 ? (
                ...
                {cart.map(item => (
                  <div key={item.id} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                      <div>
                        <div style={{ color: '#f1f5f9', fontWeight: 'bold', fontSize: '14px' }}>{item.name}</div>
                        <div style={{ color: '#64748b', fontSize: '11px', marginTop: '2px' }}>Batch: {item.batchNo}</div>
                      </div>
                      <Trash2 size={16} onClick={() => removeFromCart(item.id)} style={{ cursor: 'pointer', color: '#ef4444', flexShrink: 0 }} />
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <button onClick={() => decreaseQty(item.id)} style={qtyBtn} disabled={item.qty <= 1}><Minus size={12} /></button>
                        <span style={{ minWidth: '28px', textAlign: 'center', fontSize: '14px', fontWeight: 'bold', color: '#f1f5f9' }}>{item.qty}</span>
                        <button onClick={() => increaseQty(item.id)} style={qtyBtn} disabled={item.qty >= item.maxStock}><Plus size={12} /></button>
                        <span style={{ color: '#94a3b8', fontSize: '12px' }}>× ₹{item.mrp}</span>
                      </div>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '480px' }}>
                  <thead>
                    <tr style={{ color: '#94a3b8', fontSize: '11px', textTransform: 'uppercase', textAlign: 'left', borderBottom: '1px solid #334155' }}>
                  ...
                  <tbody>
                    {cart.map(item => (
                      <tr key={item.id} style={{ color: '#cbd5e1', borderBottom: '1px solid #334155', fontSize: '13px' }}>
                        ...
                        <td style={{ color: '#10b981', fontWeight: 'bold' }}>₹{(item.mrp * item.qty).toFixed(2)}</td>
                        <td><Trash2 size={16} onClick={() => removeFromCart(item.id)} style={{ cursor: 'pointer', color: '#ef4444' }} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div style={{
          ...
          <div style={{ borderTop: '1px solid #334155', paddingTop: '12px', marginTop: '10px' }}>
            ...
            <input
              type="number"
              value={finalAmount}
              onChange={e => { setFinalAmount(e.target.value); setRoundOff(parseFloat(e.target.value || 0) - subtotal); }}
              style={{
                width: '100%', padding: '10px', backgroundColor: '#0f172a',
                border: '2px solid #10b981', borderRadius: '8px', color: '#10b981',
          ...
          {isCreditSale && (
            <div style={{ marginTop: '12px', padding: '10px', backgroundColor: '#451a03', borderRadius: '8px', border: '1px solid #f59e0b', fontSize: '12px', color: '#fcd34d' }}>
          ...
          {cart.length > 0 && (
            <button
              onClick={() => { setCart([]); setError(''); setCustomerId(''); setFinalAmount(''); setRoundOff(0); }}
              style={{ width: '100%', marginTop: '10px', backgroundColor: 'transparent', color: '#ef4444', border: '1px solid #ef4444', padding: '10px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}
            >
              Clear Cart
            </button>
          )}
...
// ── Styles ─────────────────────────────────────────────────────────────────────
const inputGroup          = { display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: '#1e293b', padding: '10px', borderRadius: '8px', border: '1px solid #334155' };
const cleanInput          = { background: 'none', border: 'none', color: 'white', width: '100%', outline: 'none', cursor: 'pointer', fontSize: '14px' };
