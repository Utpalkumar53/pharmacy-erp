...
const Finance = () => {
  ...
  const exportCSV = () => {
    ...
    const rows = [
      ...
      ...cashBook.map(d => [d.date, fmtN(d.salesIn), fmtN(d.purchasesOut), fmtN(d.expensesOut), fmtN(d.netCash)]),
    ];
    const csv  = rows.map(r => r.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = `Finance_${MONTHS[month - 1]}_${year}.csv`; a.click();
  ...
  const printReport = () => {
    ...
    const cbRows = cashBook.map(d => `
      <tr>
        ...
        <td style="font-weight:bold;color:${d.netCash >= 0 ? '#16a34a' : '#dc2626'}">₹${fmtN(d.netCash)}</td>
      </tr>`).join('');
    const html = `<html><head><style>
      ...
      .green{color:#16a34a;font-weight:bold} .red{color:#dc2626;font-weight:bold}
      ...
      <p style="color:#94a3b8;font-size:11px;margin-top:30px">Generated by RxManager Pro — ${new Date().toLocaleString('en-IN')}</p>
    </body></html>`;
      ...
      <div className="fin-header">
        ...
        {pl && (
          <div className="fin-action-btns">
            <button onClick={exportCSV} style={s.csvBtn}><Download size={15}/> Export CSV</button>
            <button onClick={printReport} style={s.printBtn}><Printer size={15}/> Print / PDF</button>
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className="fin-card-grid">
            ...
            <div style={{
              ...
              <span style={{ ...s.cardVal, fontSize: '30px', color: pl.netProfit >= 0 ? '#10b981' : '#ef4444' }}>
                {fmt(pl.netProfit)}
              </span>
          ...
          {tab === 'pl' && (
            <div style={s.tableCard}>
              <h3 style={s.tableTitle}>Profit & Loss Statement — {MONTHS[month - 1]} {year}</h3>
                ...
                <table style={{ ...s.table, minWidth: '500px' }}>
                  <thead>
                    <tr>
                      ...
                      <th style={{ ...s.th }} className="fin-col-notes">Notes</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr style={{ backgroundColor: 'rgba(16,185,129,0.06)' }}>
                      <td colSpan={3} style={{ ...s.td, color: '#10b981', fontWeight: '700', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Income</td>
                    </tr>
                    <tr>
                      <td style={s.td}>Total Sales Revenue</td>
                      <td style={{ ...s.td, color: '#10b981', fontWeight: '700' }}>{fmt(pl.totalRevenue)}</td>
                      <td style={{ ...s.td, color: '#64748b', fontSize: '12px' }} className="fin-col-notes">All counter + hospital sales</td>
                    </tr>
                    <tr style={{ backgroundColor: 'rgba(239,68,68,0.06)' }}>
                      <td colSpan={3} style={{ ...s.td, color: '#ef4444', fontWeight: '700', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Cost of Goods</td>
                    </tr>
                    <tr>
                      <td style={s.td}>Cost of Goods Sold (COGS)</td>
                      <td style={{ ...s.td, color: '#f87171', fontWeight: '600' }}>− {fmt(pl.totalCOGS)}</td>
                      <td style={{ ...s.td, color: '#64748b', fontSize: '12px' }} className="fin-col-notes">Purchase cost of medicines sold</td>
                    </tr>
                    <tr style={{ borderTop: '1px solid #334155', backgroundColor: 'rgba(96,165,250,0.06)' }}>
                      ...
                      <td style={{ ...s.td, color: '#64748b', fontSize: '12px' }} className="fin-col-notes">Revenue − COGS</td>
                    </tr>
                    <tr style={{ backgroundColor: 'rgba(251,191,36,0.06)' }}>
                      <td colSpan={3} style={{ ...s.td, color: '#fbbf24', fontWeight: '700', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Operating Expenses</td>
                    </tr>
                    <tr>
                      <td style={s.td}>Total Operational Expenses</td>
                      <td style={{ ...s.td, color: '#fbbf24', fontWeight: '600' }}>− {fmt(pl.totalExpenses)}</td>
                      <td style={{ ...s.td, color: '#64748b', fontSize: '12px' }} className="fin-col-notes">Rent, food, transport, utilities</td>
                    </tr>
                    <tr>
                      <td style={s.td}>GST Collected (Output Tax)</td>
                      <td style={{ ...s.td, color: '#a78bfa' }}>{fmt(pl.totalGst)}</td>
                      <td style={{ ...s.td, color: '#64748b', fontSize: '12px' }} className="fin-col-notes">To be remitted to government</td>
                    </tr>
                    <tr style={{ borderTop: '2px solid #334155', backgroundColor: pl.netProfit >= 0 ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)' }}>
                      <td style={{ ...s.td, fontWeight: '700', color: 'white', fontSize: '16px' }}>NET PROFIT</td>
                      <td style={{ ...s.td, fontWeight: '700', fontSize: '20px', color: pl.netProfit >= 0 ? '#10b981' : '#ef4444' }}>{fmt(pl.netProfit)}</td>
                      <td style={{ ...s.td, color: '#64748b', fontSize: '12px' }} className="fin-col-notes">Margin: {(pl.profitMargin || 0).toFixed(1)}%</td>
                    </tr>
                    <tr>
                      <td style={s.td}>Customer Outstanding (Udhaar)</td>
                      <td style={{ ...s.td, color: '#f472b6' }}>{fmt(pl.totalOutstanding || 0)}</td>
                      <td style={{ ...s.td, color: '#64748b', fontSize: '12px' }} className="fin-col-notes">Pending collection from customers</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          {tab === 'cashbook' && (
            <div style={s.tableCard}>
              <h3 style={s.tableTitle}>Daily Cash Book — {MONTHS[month - 1]} {year}</h3>

              {/* Cash book summary mini cards */}
              <div className="fin-mini-grid">
                ...
                <div style={{ ...s.miniCard, border: `1px solid ${cbTotals.netCash >= 0 ? '#10b981' : '#ef4444'}44` }}>
                ...
                <div className="fin-table-scroll">
                  <table style={{ ...s.table, minWidth: '500px' }}>
                    ...
                    <tbody>
                      {cashBook.map((day, i) => (
                        <tr key={i} style={s.tr}>
                          <td style={{ ...s.td, color: 'white', fontWeight: '500', whiteSpace: 'nowrap' }}>
                            {new Date(day.date).toLocaleDateString('en-IN', { weekday: 'short', day: '2-digit', month: 'short' })}
                          </td>
                          <td style={{ ...s.td, color: '#10b981', fontWeight: '600', whiteSpace: 'nowrap' }}>
                            {day.salesIn > 0 ? `+ ${fmt(day.salesIn)}` : '—'}
                          </td>
                          <td style={{ ...s.td, color: '#f97316', whiteSpace: 'nowrap' }}>
                            {day.purchasesOut > 0 ? `− ${fmt(day.purchasesOut)}` : '—'}
                          </td>
                          <td style={{ ...s.td, color: '#ef4444', whiteSpace: 'nowrap' }}>
                            {day.expensesOut > 0 ? `− ${fmt(day.expensesOut)}` : '—'}
                          </td>
                          <td style={{ ...s.td, fontWeight: '700', whiteSpace: 'nowrap', color: day.netCash >= 0 ? '#10b981' : '#ef4444' }}>
                      ...
                      <tr style={{ borderTop: '2px solid #334155', backgroundColor: '#0f172a' }}>
                        ...
                        <td style={{ ...s.td, fontWeight: '700', fontSize: '15px', color: cbTotals.netCash >= 0 ? '#10b981' : '#ef4444' }}>
                          {fmt(cbTotals.netCash)}
                        </td>
                      </tr>
                    </tbody>
